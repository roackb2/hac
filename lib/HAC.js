...
var MIFeatureSelector = require("./MIFeatureSelector.js");
...
function HAC() {
	...
	this.clusterIndex = 0;
	this.lastAlive = -1;
	this.selector = new MIFeatureSelector();	
}
...
HAC.prototype = {
	...
	getClustersWithLabels: function(k, fields, featureCount) {
		this.selector = new MIFeatureSelector();	
		var finalTree = this.lastAlive;		
		var threshold = 2 * this.docs.length - k;
		var clusters = [];
		...
		finalTree.conditionalPreOrderTraverse(function(node) {
			if(node.id < threshold) {
				...
				for(var i = 0; i < node.docs.length; i++) {
					...
					cluster.docs.push(doc);
					owner.selector.addDocument(node.docs[i].terms, node.id);
				}
				clusters.push(cluster);
			}
		}, function(child) {
