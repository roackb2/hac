...
HAC.prototype = {
	...
	getMeasure: function(clusters, method, beta, showRawScore) {
		...
		_.forEach(clusters, cluster => {
			...
			var clustClasses = []
			_.forEach(cluster.docs, doc => {
				var item = clustClasses.find(x => {
	...
	getDistribution: function(clusters) {
		var res = "";
		_.forEach(clusters, (cluster, i) => {
			res += "cluster " + i + "\n";
			cluster.docs.sort((item1, item2) => {
...
function combination(n, r) {
	...
	var denominators = [];
    var res = 1;
    _.forEach(_.range(1, n + 1), x => {
    	numerators.push(x);
    })
    _.forEach(_.range(1, r + 1), x => {
    ...
    _.forEach(numerators, x => {
    	res *= x;
    })
    _.forEach(denominators, x => {
    	res /= x;
    })
    return res;
}
